...
window.initializePlayground = async opts => {
	...
	async function renderTree() {
		...
		for (let i = 0; ; i++) {
			...
			if (cursor.nodeIsMissing) {
				const nodeTypeText = cursor.nodeIsNamed
					? cursor.nodeType
					: `"${cursor.nodeType}"`;
			...
			if (visitedChildren) {
				...
				if (displayName) {
					...
					row =
						`<div class="tree-row">${'  '.repeat(indentLevel)}${fieldName}` +
	...
	function handleCursorMovement() {
		...
		if (treeRows) {
			if (treeRowHighlightedIndex !== -1) {
				...
				if (row)
					treeRows[treeRowHighlightedIndex] = row.replace('highlighted', 'plain');
			}
			...
			if (treeRowHighlightedIndex !== -1) {
				const row = treeRows[treeRowHighlightedIndex];
				if (row)
					treeRows[treeRowHighlightedIndex] = row.replace('plain', 'highlighted');
			}
	...
	function handleTreeClick(event) {
		if (event.target.tagName === 'A') {
			...
			const [startRow, startColumn, endRow, endColumn] = event.target.dataset.range
				.split(',')
				.map(n => parseInt(n));
			codeEditor.focus();
